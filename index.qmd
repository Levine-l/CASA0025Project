...
Based on precise location data from Amnesty International (2025) and ASPI (2025), our final compiled dataset includes confirmed points, suspected points, and control points on GitHub: [scam_points_update.csv](https://github.com/Levine-l/CASA0025Project/blob/main/data/processed/scam_points_update.csv){target="_blank"}
...
pixels with confirmed Cambodia reference samples. Pixels above the p97 similarity 
...
Pixels above the p97 similarity threshold are retained as broad candidates.
...
Stage 2 converts candidates into 500 m zones and extracts NDVI/NDBI change, VIIRS nighttime 
light change, distance variables and candidate zone area. 
...
Spatial context is represented by proximity and size variables:

$$
d_{\text{confirmed}}(i)=\min_{c \in C}\text{dist}(Z_i,c)
$$

$$
d_{\text{border}}(i)=D_B(\text{centroid}(Z_i))
$$
...
- $Z_i$ is the 500 m candidate zone, 
- $C$ is the set of confirmed sites， 
- $D_B$ is the raster distance surface generated from national borders.


**Validation**

Validation compares outputs with suspected, non-reference confirmed, held out 
confirmed and control points.
...
The first step loads the 2024 Google Satellite Embedding image for the Cambodia–Vietnam AOI 
and samples vectors from three confirmed reference sites. Each reference vector is converted 
into a constant image and compared pixel by pixel with the full embedding image by dot product 
similarity. The mean of the reference similarity images becomes the Stage 1 similarity surface.
...
Pixels above the p97 threshold are retained as broad candidates. This layer is designed for 
high recall, so it is passed to Stage 2 for refinement.


**Stage 2.1: Computing Candidate-Level Metrics**

Stage 2 uses 500 m candidate zones to build candidate-level metrics. Sentinel-2 and VIIRS 
composites produce NDVI/NDBI and night-time light change indicators.
...
Border distance and area are added as spatial context fields. Border distance uses a raster 
distance surface with `fastDistanceTransform()`, avoiding repeated vector distance calculations.
...
Precomputed metrics are converted into development and activity flags, then combined into high, 
medium and low priority tiers.
...
The operational shortlist applies stricter proximity, night-time light and growth conditions 
for default display.
